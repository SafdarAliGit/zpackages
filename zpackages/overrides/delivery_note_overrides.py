...
def custom_on_submit(doc, method):
        source_name = frappe.get_doc("Delivery Note", doc.name)
        delivery_note_type = source_name.name
        if not delivery_note_type.startswith("SR"):
            try:
                si = frappe.new_doc("Sales Invoice")
                si.posting_date = source_name.posting_date
                si.customer = source_name.customer
                si.delivery_note = source_name.name
