...
frappe.ui.form.on('Stock Entry', {
    setup: function (frm) {
        ...
        frm.set_query('work_order', function () {
            return {
                filters: [['Work Order', 'docstatus', '=', 1], ['Work Order', 'qty', '>', '`tabWork Order`.produced_qty'], ['Work Order', 'company', '=', frm.doc.company]]
            }
        });

        frm.set_query('outgoing_stock_entry', function () {
            return {
                filters: [['Stock Entry', 'docstatus', '=', 1], ['Stock Entry', 'per_transferred', '<', '100'],]
            }
        });
        ...
        frm.set_query('source_warehouse_address', function () {
            return {
                filters: {
                    link_doctype: 'Warehouse', link_name: frm.doc.from_warehouse
                }
            }
        });
        ...
        frm.set_query('target_warehouse_address', function () {
            return {
                filters: {
                    link_doctype: 'Warehouse', link_name: frm.doc.to_warehouse
                }
            }
        });
        ...
        frappe.db.get_value('Stock Settings', {name: 'Stock Settings'}, 'sample_retention_warehouse', (r) => {
            if (r.sample_retention_warehouse) {
                var filters = [["Warehouse", 'company', '=', frm.doc.company], ["Warehouse", "is_group", "=", 0], ['Warehouse', 'name', '!=', r.sample_retention_warehouse]]
        ...
        frm.set_query('batch_no', 'items', function (doc, cdt, cdn) {
            ...
            if (!item.item_code) {
                ...
                if (in_list(["Material Transfer for Manufacture", "Manufacture", "Repack", "Send to Subcontractor"], doc.purpose)) {
                    var filters = {
                        'item_code': item.item_code, 'posting_date': frm.doc.posting_date || frappe.datetime.nowdate()
                    }
                } else {
                    var filters = {
                ...
                return {
                    query: "erpnext.controllers.queries.get_batch_no", filters: filters
                }
            }
        });
    ...
    outgoing_stock_entry: function (frm) {
        frappe.call({
            doc: frm.doc, method: "set_items_for_stock_in", callback: function () {
                refresh_field('items');
            }
        });
    },
            ...
            if (frm.doc.add_to_transit && frm.doc.purpose == 'Material Transfer' && frm.doc.per_transferred < 100) {
                frm.add_custom_button(__('End Transit'), function () {
                    frappe.model.open_mapped_doc({
                        method: "erpnext.stock.doctype.stock_entry.stock_entry.make_stock_in_entry", frm: frm
                    })
                });
            }
            ...
            if (frm.doc.per_transferred > 0) {
                frm.add_custom_button(__('Received Stock Entries'), function () {
                    frappe.route_options = {
                        'outgoing_stock_entry': frm.doc.name, 'docstatus': ['!=', 2]
                    };
        ...
        if (frm.doc.docstatus === 0) {
            frm.add_custom_button(__('Transit Entry'), function () {
                erpnext.utils.map_current_doc({
                    ...
                    get_query_filters: {
                        docstatus: 1, purpose: "Material Transfer", add_to_transit: 1,
                    }
                })
            }, __("Get Items From"));
        }
    ...
    set_serial_no: function (frm, cdt, cdn, callback) {
        ...
        var args = {
            'item_code': d.item_code, 'warehouse': cstr(d.s_warehouse), 'stock_qty': d.transfer_qty
        };
        frappe.call({
            method: "erpnext.stock.get_item_details.get_serial_no", args: {"args": args}, callback: function (r) {
                if (!r.exe && r.message) {
    ...
    make_retention_stock_entry: function (frm) {
        frappe.call({
            method: "erpnext.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse", args: {
                "company": frm.doc.company, "items": frm.doc.items
            }, callback: function (r) {
                if (r.message) {
    ...
    set_basic_rate: function (frm, cdt, cdn) {
        ...
        if (item.item_code || item.serial_no) {
            frappe.call({
                method: "erpnext.stock.utils.get_incoming_rate", args: {
                    args: args
                }, callback: function (r) {
    ...
    get_warehouse_details: function (frm, cdt, cdn) {
        ...
        if (!child.bom_no) {
            frappe.call({
                method: "erpnext.stock.doctype.stock_entry.stock_entry.get_warehouse_details", args: {
                    "args": {
    ...
    show_bom_custom_button: function (frm) {
        if (frm.doc.docstatus === 0 && ['Material Issue', 'Material Receipt', 'Material Transfer', 'Send to Subcontractor'].includes(frm.doc.purpose)) {
    ...
    get_items_from_bom: function (frm) {
        ...
        let fields = [{
            "fieldname": "bom", "fieldtype": "Link", "label": __("BOM"), options: "BOM", reqd: 1, get_query: filters()
        }, {
            "fieldname": "source_warehouse", "fieldtype": "Link", "label": __("Source Warehouse"), options: "Warehouse"
        }, {
            "fieldname": "target_warehouse", "fieldtype": "Link", "label": __("Target Warehouse"), options: "Warehouse"
        }, {
        ...
        let d = new frappe.ui.Dialog({
            title: __("Get Items from BOM"), fields: fields
        });
        d.get_input("fetch").on("click", function () {
            ...
            frappe.call({
                method: "erpnext.manufacturing.doctype.bom.bom.get_bom_items", args: values, callback: function (r) {
                    if (!r.message) {
    ...
    calculate_total_additional_costs: function (frm) {
        const total_additional_costs = frappe.utils.sum((frm.doc.additional_costs || []).map(function (c) {
    ...
    add_to_transit: function (frm) {
        if (frm.doc.purpose == 'Material Transfer') {
            var filters = {
                'is_group': 0, 'company': frm.doc.company
            }
    ...
    set_transit_warehouse: function (frm) {
        if (frm.doc.add_to_transit && frm.doc.purpose == 'Material Transfer' && !frm.doc.to_warehouse && frm.doc.from_warehouse) {
...
frappe.ui.form.on('Stock Entry Detail', {
    ...
    uom: function (doc, cdt, cdn) {
        ...
        if (d.uom && d.item_code) {
            return frappe.call({
                method: "erpnext.stock.doctype.stock_entry.stock_entry.get_uom_details", args: {
                    item_code: d.item_code, uom: d.uom, qty: d.qty
                }, callback: function (r) {
                    if (r.message) {
    ...
    item_code: function (frm, cdt, cdn) {
        ...
        if (d.item_code) {
            ...
            return frappe.call({
                doc: frm.doc, method: "get_item_details", args: args, callback: function (r) {
                    if (r.message) {
                        var d = locals[cdt][cdn];
...
var validate_sample_quantity = function (frm, cdt, cdn) {
    ...
    if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
        frappe.call({
            method: 'erpnext.stock.doctype.stock_entry.stock_entry.validate_sample_quantity', args: {
                batch_no: d.batch_no, item_code: d.item_code, sample_quantity: d.sample_quantity, qty: d.transfer_qty
            }, callback: (r) => {
...
erpnext.stock.StockEntry = class StockEntry extends erpnext.stock.StockController {
    setup() {
        ...
        this.frm.set_query("purchase_order", function () {
            return {
                "filters": {
                    "docstatus": 1, "is_old_subcontracting_flow": 1, "company": me.frm.doc.company
                }
            };
        });
        ...
        this.frm.set_query("subcontracting_order", function () {
            return {
                "filters": {
                    "docstatus": 1, "company": me.frm.doc.company, "status": ["not in", ["Completed", "Closed"]]
                }
            };
        });
        ...
        this.frm.fields_dict.items.grid.get_field('expense_account').get_query = function () {
            if (erpnext.is_perpetual_inventory_enabled(me.frm.doc.company)) {
                return {
                    filters: {
                        "company": me.frm.doc.company, "is_group": 0
                    }
                }
            }
        }
    ...
    clean_up() {
        ...
        if (this.frm.doc.work_order && in_list(["Manufacture", "Material Transfer for Manufacture", "Material Consumption for Manufacture"], this.frm.doc.purpose)) {
    ...
    get_items() {
        var me = this;
        if (!this.frm.doc.fg_completed_qty || !this.frm.doc.bom_no) frappe.throw(__("BOM and Manufacturing Quantity are required"));

        if (this.frm.doc.work_order || this.frm.doc.bom_no) {
            // if work order / bom is mentioned, get items
            return this.frm.call({
                doc: me.frm.doc, freeze: true, method: "get_items", callback: function (r) {
    ...
    work_order() {
        ...
        return frappe.call({
            method: "erpnext.stock.doctype.stock_entry.stock_entry.get_work_order_details", args: {
                work_order: me.frm.doc.work_order, company: me.frm.doc.company
            }, callback: function (r) {
                if (!r.exc) {
                    ...
                    if (me.frm.doc.purpose == "Material Transfer for Manufacture" && !me.frm.doc.to_warehouse) me.frm.set_value("to_warehouse", r.message["wip_warehouse"]);
    ...
    add_excise_button() {
        if (frappe.boot.sysdefaults.country === "India") this.frm.add_custom_button(__("Excise Invoice"), function () {
    ...
    toggle_related_fields(doc) {
        ...
        if (doc.purpose == 'Send to Subcontractor') {
            doc.customer = doc.customer_name = doc.customer_address = doc.delivery_note_no = doc.sales_invoice_no = null;
        } else {
            doc.customer = doc.customer_name = doc.customer_address = doc.delivery_note_no = doc.sales_invoice_no = doc.supplier = doc.supplier_name = doc.supplier_address = doc.purchase_receipt_no = doc.address_display = null;
        }
        ...
        this.frm.toggle_display(["additional_costs", "total_additional_costs", "additional_costs_section"], doc.purpose != 'Material Issue');
...
erpnext.stock.select_batch_and_serial_no = (frm, item) => {
    let get_warehouse_type_and_name = (item) => {
        ...
        if (frm.fields_dict.from_warehouse.disp_status === "Write") {
            ...
            return {
                type: 'Source Warehouse', name: value
            };
        } else {
            value = cstr(item.t_warehouse) || '';
            return {
                type: 'Target Warehouse', name: value
            };
        }
    }
    ...
    frappe.require("assets/erpnext/js/utils/serial_no_batch_selector.js", function () {
        ...
        frm.batch_selector = new erpnext.SerialNoBatchSelector({
            frm: frm, item: item, warehouse_details: get_warehouse_type_and_name(item),
        });
    });
